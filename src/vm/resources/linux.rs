...
impl LinuxCgroupLimiter {
    ...
    /// Placeholder when cgroup fs is unavailable (post-spawn becomes no-op).
    ...
    pub fn is_available(&self) -> bool {
        !self.cgroup_root.as_os_str().is_empty() && self.cgroup_root.exists()
    }
    ...
    async fn apply_cpu_limits(&self, process_id: Uuid, cpu_cores: u16) -> Result<(), AppError> {
        match self.cgroup_version {
            CgroupVersion::V2 => {
                ...
                fs::write(&cpu_max_path, cpu_max.as_bytes())
                    .await
                    .map_err(|e| {
            ...
            CgroupVersion::V1 => {
                ...
                fs::write(&quota_path, quota.to_string().as_bytes())
                    .await
                    .map_err(|e| {
    ...
    async fn apply_memory_limits(&self, process_id: Uuid, memory_mb: u32) -> Result<(), AppError> {
        ...
        match self.cgroup_version {
            CgroupVersion::V2 => {
                ...
                fs::write(&memory_max_path, memory_bytes.to_string().as_bytes())
                    .await
                    .map_err(|e| {
            ...
            CgroupVersion::V1 => {
                ...
                fs::write(&memory_limit_path, memory_bytes.to_string().as_bytes())
                    .await
                    .map_err(|e| {
                        AppError::ConfigError(format!(
                            "Failed to write memory.limit_in_bytes: {}",
                            e
                        ))
                    })?;
            }
    ...
    async fn add_process_to_cgroup(&self, process_id: Uuid, pid: u32) -> Result<(), AppError> {
        ...
        match self.cgroup_version {
            ...
            CgroupVersion::V1 => {
                ...
                fs::write(&tasks_path, pid.to_string().as_bytes())
                    .await
                    .map_err(|e| AppError::ConfigError(format!("Failed to write tasks: {}", e)))?;
            }
        }
    ...
    async fn get_memory_usage(&self, process_id: Uuid) -> Result<u32, AppError> {
        ...
        match self.cgroup_version {
            CgroupVersion::V2 => {
                ...
                let usage_bytes = fs::read_to_string(&memory_current_path)
                    .await
                    .map_err(|e| {
                        AppError::ConfigError(format!("Failed to read memory.current: {}", e))
                    })?
                    .trim()
                    .parse::<u64>()
                    .map_err(|e| {
                        AppError::ConfigError(format!("Failed to parse memory.current: {}", e))
                    })?;
            ...
            CgroupVersion::V1 => {
                ...
                let usage_bytes = fs::read_to_string(&memory_usage_path)
                    .await
                    .map_err(|e| {
                        AppError::ConfigError(format!(
                            "Failed to read memory.usage_in_bytes: {}",
                            e
                        ))
                    })?
                    .trim()
                    ...
                    .map_err(|e| {
                        AppError::ConfigError(format!(
                            "Failed to parse memory.usage_in_bytes: {}",
                            e
                        ))
                    })?;
    ...
    pub async fn apply_limits(
        ...
        if limits.memory_mb > 0 {
            self.apply_memory_limits(process_id, limits.memory_mb)
                .await?;
        }
...
/// Pre-spawn hints (cgroups need PID — applied in `apply_limits_post_spawn`).
pub async fn apply_linux_pre_spawn(
    command: &mut Command,
    limits: &ResourceLimits,
) -> Result<(), AppError> {
