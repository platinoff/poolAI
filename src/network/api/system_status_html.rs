//! HTML сторінка для `GET /api/v1/status` при `Accept: text/html`.
//!
//! Розмітка лишається поза [`crate::services::system_service::SystemService`], щоб сервіс лишався лише для JSON/оркестрації.

use crate::services::system_service::StatusResponse;

pub(super) fn status_page_html(status: &StatusResponse) -> String {
    format!(
        r#"
<!DOCTYPE html>
<html lang='en'>
