...
**Версія репозиторію:** 0.2.2 (`Cargo.toml`). **Оновлено:** 2026-04-13 (FM-012 **Partial**: i18n у `i18n_core.js` + enterprise admin **monitoring/config/security/instances/topology** + **tenants/VM/workers/libs/users/RAID** (+ dashboard/audit у shell); **login/refresh** — опційне поле **`bootstrap_default_admin`**; UI — банер першого входу для дефолтного `admin`; enterprise **Telegram** callback — частково HMAC/`auth_date`/allowlist; узгоджено з **`FUNCTION_MANAGEMENT`** / **HANDOFF**).
