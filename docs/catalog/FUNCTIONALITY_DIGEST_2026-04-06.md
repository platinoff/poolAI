...
**Версія репозиторію:** 0.2.2 (`Cargo.toml`). **Оновлено:** 2026-04-12 (FM-012 **Partial**: i18n у `i18n_core.js`; узгоджено з **`FUNCTION_MANAGEMENT`** / **HANDOFF**).
...
| **UI** | `ui/` | Вбудована веб-адмінка (дашборди, теми, доступність). **FM-012 Partial:** i18n **UA/EN** (`i18n_core.js`), `/ui/auth`, layout `/ui/*`, write-flow **Workers/Libs/VM/RAID**, enterprise **admin** (заголовки сторінок, dashboard, audit). Мапінг JSON адмінки → екран: `docs/development/ADMIN_UI_JSON_CONTRACTS.md`. |
