...
**Версія репозиторію:** 0.2.2 (`Cargo.toml`). **Оновлено:** 2026-05-27 (PH-S68: `galaxy_pricing_oracle` stub; PH-S67 Galaxy Grid modules zріз).
...
| **Pricing oracle stub** | `src/grid/galaxy_pricing_oracle.rs` | unit keys; `floor(market_min×0.9)` usd_micro; TTL/SWR cache; `POOLAI_GALAXY_PRICE_*` env (§4.2) | unit tests (`galaxy_pricing_oracle`) |
