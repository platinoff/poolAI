...
| **Pricing oracle (stub + L2 fallback)** | `src/grid/galaxy_pricing_oracle.rs` | unit keys; TTL/SWR cache (L1 stale); L2 fallback + FORCE_FALLBACK; metrics `galaxy_pricing_forced_fallback_total`, `galaxy_pricing_stale_served` (PH-S81/S83) | unit tests (`galaxy_pricing_oracle`) |
