...
| FM-005 | P3 | Спрощення handler’ів до `Result<T, AppError>` / `HttpAppError` де доречно (без зміни стабільних `error.code`) | Partial | NEXT_STEPS P3; **`rewards`**, **`workers`**, **`admin`**, **`system`**; **`libraries`** (`GET /libraries`, **`GET /libraries/{name}`** → `Result<_, LibHttpErr>`); **`instances`** (`GET /instance`, **`GET /instance/{id}`**, `GET /state` → `Result<_, InstanceJsonError>`); **`/rewards/progress/*`** — `api_json_error` (`NOT_FOUND`) |
