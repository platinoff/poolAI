...
**Оновлено:** 2026-05-25 (PH-S47 ✅ CI #1213; §5.11 PH-S37…S44; FM-041 Deferred).
...
| 1 | **PH-S37** Visual baselines (Linux CI) | **§5.10** | **Відкрито** — Actions workflow → merge PR Linux PNG |
...
**Наступна сесія (код/CI):** **PH-S37** (Linux PNG merge) → **PH-S44**. **Не стартувати без інфра:** PH-S35, PH-S36 без запиту.
...
|---|--------|--------|---------|------|
| — | **PH-S47** | CI green; локально `cargo test-ci` + gap audit | `ci.yml`, tests | **✅** — CI #1213 (`0fe21bf1`) ubuntu+windows+openapi-gap |
| 1 | **PH-S37** | Linux visual baselines (`visual.spec.ts-snapshots`) | §5.10, `VISUAL_REGRESSION_E2E.md` | **Відкрито** — workflow `update-visual-baselines.yml` |
...
| 6 | **PH-S45** | E2E stability: VM create modal + axe `/ui/admin/audit` | `admin.spec.ts`, `a11y.spec.ts` | **Відкрито** |
| 7 | **PH-S38** | Job scheduler hardening + on-chain submit epics | §5.10, Architect L236, `src/job/` | **Відкрито** |
| 8 | **PH-S46** | Solana on-chain program / events (post FM-024 mock RPC) | §5.7, `SOLANA_ADAPTER_CONCEPT` | **Відкрито** |
...
**Наступна сесія:** **PH-S37** (Linux PNG merge) → **PH-S44** · [`NEXT_SESSION_PROMPT.md`](../development/NEXT_SESSION_PROMPT.md).
