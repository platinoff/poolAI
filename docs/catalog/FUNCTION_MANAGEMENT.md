...
| FM-012 | UI / Auth UX | Апгрейд `/ui` і `/ui/admin/*`: **Partial** — i18n **UA/EN** (`i18n_core.js`, `/ui/auth`, layout `/ui/*`, write-flow, **admin** — `admin_layout`, **dashboard**, **audit**, **monitoring**, **config**, **security**, **instances**, **topology**, **tenants**, **VM**, **workers**, **libs**, **users**, **RAID**; **далі** — решта `/ui/*`, Telegram hardening, audit/allowlist | Partial | `src/ui/i18n_core.js`, `src/ui/mod.rs`, `src/ui/admin/`, `src/ui/admin_common.js`, `src/network/enterprise_api/oauth.rs` |
...
| 4 | Product UX | **FM-012** | **Зроблено (Partial):** як вище + **admin** `admin_layout`, **dashboard**, **audit**, **monitoring**, **config**, **security**, **instances**, **topology**, **tenants**, **VM**, **workers**, **libs**, **users**, **RAID** (ключі `admin.*` / `workers.*` / `vm.*` тощо, динамічний текст через `poolaiT`). **Далі:** решта `/ui/*` за потреби; Telegram OAuth hardening (allowlist, audit). |
