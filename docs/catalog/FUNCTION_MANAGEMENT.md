...
**Оновлено:** 2026-04-10 (FM-005: `topology`, `vm`, `rewards/progress` на `HttpAppError`; таблиця **FM-***; **§5.1** без зміни порядку; узгоджено зі [`STABLE_STATE_SUMMARY.md`](../status/STABLE_STATE_SUMMARY.md); **якість:** Clippy за матрицями `ci.yml` — звіряти локально після змін)  
...
| FM-005 | P3 | Спрощення handler’ів до `Result<T, AppError>` / `HttpAppError` де доречно (без зміни стабільних `error.code`) | Partial | NEXT_STEPS P3; зроблено: **`libraries`**, **`instances`** (GET), **`rewards`** (`/rewards/progress/{id}` → `HttpAppError`), **`topology`** (усі GET), **`vm`** (усі маршрути через `HttpAppError`); лишаються **`workers`** (POST/DELETE), **`system`** (login/config), **`users`**, **`ui`**, **`raid`**, **`ai_ml`**, **`completions`**, **`enterprise`**, тощо |
