...
**Оновлено:** 2026-04-10 (FM-005: `workers` POST/DELETE, `instances` previews/create/delete, `system` config/refresh, `completions`; `AppError::RestError` для стабільних не-SCREAMING кодів; таблиця **FM-***; **§5.1** без зміни порядку)  
...
| FM-005 | P3 | Спрощення handler’ів до `Result<T, AppError>` / `HttpAppError` де доречно (без зміни стабільних `error.code`) | Partial | NEXT_STEPS P3; зроблено також: **`workers`**, **`instances`** (previews/create/delete), **`system`** (`/config`, `/refresh` без токена), **`completions`** (`internal_error` через `RestError`); лишаються **`users`**, **`ui`**, **`raid`**, **`raid_admin`**, **`raid_http`**, **`ai_ml`**, **`enterprise`**, login/refresh `(status, Json)` з auth |
