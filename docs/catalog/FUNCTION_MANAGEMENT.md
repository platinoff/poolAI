...
| 4 | Product UX | **FM-012** | **Зроблено (Partial):** як вище + **admin** `admin_layout`, **dashboard**, **audit**, **monitoring**, **config**, **security**, **instances**, **topology**, **tenants**, **VM**, **workers**, **libs**, **users**, **RAID** (ключі `admin.*` / `workers.*` / `vm.*` тощо, динамічний текст через `poolaiT`); банер першого входу для сідженого `admin` + **`bootstrap_default_admin`** у **`login`/`refresh`**; Telegram widget callback — перевірка підпису, `auth_date`, allowlist, audit. **Далі:** решта `/ui/*` за потреби; подальше загострення Telegram/OAuth за політикою. |
