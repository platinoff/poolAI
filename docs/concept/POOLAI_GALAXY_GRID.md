...
- **Live provider HTTP fetch (PH-S102/S900 ✅):** `fetch_live_provider_quotes` — `POOLAI_GALAXY_PRICING_TIMEOUT_MS` clamp 100–30000 ms, connect + request timeout; metrics `galaxy_pricing_provider_timeouts_total`, `galaxy_pricing_provider_errors_total`; admin wasm freshness strip (PH-S902); `pricing_depth` on `GET /api/v1/grid/pricing-metrics` (PH-S904).
- **Trust score SQLite persist (PH-S910…S919 ✅):** `POOLAI_TRUST_SCORE_STORE=sqlite` + `POOLAI_TRUST_SCORE_DATA_DIR` → `trust_scores.db` (migrates legacy JSON); payout gate uses persisted score when metrics omit `trust_score` (PH-S911); `trust_persist_depth` on `GET /api/v1/grid/trust-metrics` (PH-S913/S914); admin wasm `renderGridTrustPersistStrip` (PH-S912).
