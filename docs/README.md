...
- **Unified API errors (P3 / FM-005)** — `src/network/json_errors.rs` (**`HttpAppError`**, **`RestError`**), **`auth.rs`**, **`ws.rs`**, **`rate_limit.rs`**; залишок міграції — **`enterprise_api/`** (див. **FM-005** у [catalog/FUNCTION_MANAGEMENT.md](./catalog/FUNCTION_MANAGEMENT.md)). Деталі — [development/HANDOFF_NEW_SESSION.md](./development/HANDOFF_NEW_SESSION.md).
