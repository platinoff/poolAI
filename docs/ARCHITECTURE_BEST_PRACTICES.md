...
- Axum: `AppError` та `HttpAppError` (опційно `ErrorContext` / override статусу) реалізують `IntoResponse` у `network::json_errors` — зручно для `Result<T, AppError>` у handler’ах; `HttpAppError` реекспортується з `network::api::common`. Приклади: `network/api/rewards.rs` (успішні GET → `Result<Json<_>, AppError>`; **`/rewards/progress/*`** лишається з `api_json_error` для стабільного `NOT_FOUND`), **`network/api/workers.rs`** (`GET /workers` → `Result<AxumJson<Vec<WorkerInfo>>, AppError>`), **`network/api/admin.rs`** (`GET /admin/overview` → `Result<Json<AdminOverview>, AppError>`), **`network/api/system.rs`** (`GET /health`, `/metrics`, `/models`, `/gpu` → `Result<Json<_>, AppError>`; **`/status`**, **`/config`**, login — без змін).
