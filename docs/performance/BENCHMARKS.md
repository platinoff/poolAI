...
| `galaxy_fee_split_benchmarks` | `cargo bench -j 1 --bench galaxy_fee_split_benchmarks` | **PH-S58 / PH-S784:** `split_gross_payment` (1 SOL, secondary min/max bps). Local micro-bench; not in weekly [`benchmarks.yml`](../../.github/workflows/benchmarks.yml) (optional on-demand). |
