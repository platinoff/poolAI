...
**⚠️ Важливо:** Rust doc-tests створюють тимчасові бінарники в `C:\msys64\tmp\rustdoctest*\`, тому потрібно виключити також цю директорію.

### Варіант 1: Виключити папки `target/` та тимчасові директорії (рекомендовано)
...
6. Додайте повні шляхи до виключень:
   ```
   S:\rust\poolAI\target
   C:\msys64\tmp
   ```
   Або використайте pattern для тимчасових doc-test файлів:
   ```
   C:\msys64\tmp\rustdoctest*
   ```
7. Натисніть **OK** і перезапустіть Avast

**Примітка:** Якщо ви використовуєте інший шлях до MSYS2 (не `C:\msys64`), замініть його на ваш шлях.
