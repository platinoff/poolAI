...
| Topology | `topology.rs` | `GET /api/v1/topology`, `/nodes`, `/latency`, `/nodes/{id}` | overview keys; nodes map + resource fields; `latency_matrix`; node detail modal fields. | `topology_*`, `topology_latency_*`, `topology_node_detail_*` |
