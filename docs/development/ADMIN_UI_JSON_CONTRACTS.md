...
| Topology | `topology.rs` | `GET /api/v1/topology`, `/nodes`, `/latency`, `/nodes/{id}`; **WS** `subscribe_topology` on `/api/v1/ws/metrics` → `topology_update` (PH-S22 live refresh) | overview keys; **SVG graph** (`#topology-graph-svg`, `PoolAiTopologyGraph`); **heatmap** `#topology-latency-heatmap`; tables + node modal. | `topology_*`, `topology_latency_*`, `topology_node_detail_*`; FM-037 unit `topology_page_includes_graph_*` |
...
| | | `GET …/metrics/burst`, `…/smallworld` | **`{ "metrics": { … } }`**: burst — `total_artifacts`, `artifacts_in_burst`, `total_requests`, `burst_threshold_rps`, `base_replication_factor`, `max_replication_factor`; smallworld — `total_artifacts`, `total_nodes`, `avg_clustering_coefficient`, `target_clustering_coefficient`, `base_replication_factor`. UI: [`RAID_ADMIN_METRICS.md`](./RAID_ADMIN_METRICS.md). | `raid_admin_burst_metrics_*`, `raid_admin_smallworld_metrics_*` |
