...
- **Priority 3 / FM-005 (HTTP-шар)**: `json_errors.rs` — **`HttpAppError`**, **`IntoResponse`**, `http_status_for_app_error`; `core/error.rs` — **`AppError::RestError`**. Міграція **`HttpAppError`/`RestError`**: **`users`**, **`ui`**, **`ai_ml`**, **`workers`**, **`instances`**, **`libraries`**, **`vm`**, **`topology`**, **`rewards`**, **`system`**, **`completions`**, **`admin`**, **`raid`**, **`raid_http`**, **`raid_admin`** (**`raid_api_err`**), **`network/enterprise_api/`** (**`enterprise_err`** / **`enterprise_json_err`** → **`HttpAppError`**). Залишок: **`check_permission`**, **`login` / `refresh`** — як раніше.
...
2. **HTTP (FM-005, Partial)** — закінчити **`login`/`refresh`** і **`check_permission`** на **`HttpAppError`/`RestError`** де доречно; не змінювати стабільні **`error.code`** без рішення. **`network/enterprise_api/`** — мігровано на **`HttpAppError`** (2026-04-10).
