...
- **`src/services/`**: `raid_service`, `vm_service`, `library_service`, **`discovery_service`**, **`topology_service`** (`/api/v1/topology/*` через `AppState::topology_manager`), `enterprise_service`, `cloud_service`, `admin_service` + `GET /api/v1/admin/overview` (`src/network/api/admin.rs`).
...
2. **P2** — REST `/raid/*` (workers, events, snapshot, …) уже через **`RaidService`**; **`/api/v1/discovery/*`** — **`DiscoveryService`**; **`/api/v1/topology/*`** — **`TopologyService`**; опційно далі — тонкі distributed handlers у `raid_distributed_handlers` vs сервіс.
