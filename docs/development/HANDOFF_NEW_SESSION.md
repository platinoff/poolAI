...
**Оновлено:** 2026-04-10 (кроки 1–12; **FM-005** у доках узгоджено з кодом: **`HttpAppError`** + **`AppError::RestError`**; залишок міграції — **`raid*`**, **`enterprise_api/`**; **§5.1** — [`FUNCTION_MANAGEMENT.md`](../catalog/FUNCTION_MANAGEMENT.md); **«Операційний порядок»** — [`NEXT_STEPS_ARCHITECT`](./NEXT_STEPS_ARCHITECT_2026-03-17.md); **FM-007/008**; **FM-011**; Clippy / [`STABLE_STATE_SUMMARY.md`](../status/STABLE_STATE_SUMMARY.md))  
...
- **Priority 3 / FM-005 (HTTP-шар)**: `json_errors.rs` — **`HttpAppError`**, **`IntoResponse`**, `http_status_for_app_error`; `core/error.rs` — **`AppError::RestError { code, message }`** для стабільних REST-кодів + решта **`AppError`**. Міграція на **`HttpAppError`/`RestError`** (узгоджено з доками): **`users`**, **`ui`**, **`ai_ml`**, **`workers`**, **`instances`**, **`libraries`**, **`vm`**, **`topology`**, **`rewards`**, **`system`**, **`completions`**, **`admin`**, тощо. Ще на **`api_json_error`** / **`api_error_response`**: **`raid.rs`**, **`raid_http.rs`**, **`raid_admin.rs`**, **`network/enterprise_api/`**; **`check_permission`** лишає **`(StatusCode, Json)`**; **`login` / `refresh`** — як раніше.
...
2. **HTTP (FM-005, Partial)** — закінчити перехід **`raid*`** і **`enterprise_api/`** на **`HttpAppError`/`RestError`** там, де зараз **`api_json_error`**; не змінювати стабільні **`error.code`** без рішення; **`login`/`refresh`** і RBAC **`check_permission`** — окремо.
