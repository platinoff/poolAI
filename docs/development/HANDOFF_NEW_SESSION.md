...
2. **P2** — REST `/raid/*` (workers, events, snapshot, …) уже через **`RaidService`**; HTTP-мапінг помилок винесено в **`network/api/raid_http.rs`**. **Наступний великий шматок без окремого доменного сервісу:** тонкий **`network/api/ui.rs`** (enterprise dashboards + themes + components, опційно `UiService`). Distributed wire (`/raid/distributed/*`) — **`RaidDistributedProtocolService`** + тонкий **`raid_distributed_handlers`**; **`POST /v1/chat/completions`** — **`ChatCompletionService`** + тонкий **`completions.rs`**; **`/api/v1/status`**, **`/health`**, **`/metrics`**, **`/models`**, **`/gpu`**, **`/login`**, **`/config`** (GET/PUT) — **`SystemService`** (тонкий **`system.rs`**, без переносу великої HTML-сторінки статусу); **`/api/v1/discovery/*`** — **`DiscoveryService`**; **`/api/v1/topology/*`** — **`TopologyService`**; **`/api/v1/workers`** — **`WorkerPoolService`**; **`/api/v1/rewards/*`** — **`RewardsService`** + **`AppState::rewards_engine`** (той самий `Arc`, що `rewards::shared_reward_engine`, `main`: `attach_rewards_engine`); опційно далі — тонкі distributed handlers у `raid_distributed_handlers` vs сервіс.
