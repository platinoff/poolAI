...
**Оновлено:** 2026-05-18 ([`AUTO_RUN_SESSION_2026-06-20.md`](./AUTO_RUN_SESSION_2026-06-20.md) — FM-019 WCAG 2.2 pa11y S10).

**Автономний прогін:** [`AUTO_RUN_SESSION_2026-06-20.md`](./AUTO_RUN_SESSION_2026-06-20.md). **FM-003 §4** BLOCKED.

**FM-003:** dev stand ✅ (`bin/run-virtual-node-dev.*`, `verify-dev-stand.*`, `core::dev_stand`). Real LAN §4 — **BLOCKED** (два фізичні хости).

**FM-016 ✅:** virtual nodes + `poolai-worker`. **FM-016+ ✅:** bind/webhook/store. **FM-016++ ✅:** `poolai-telegram-bot`. **FM-016+++ ✅:** pool join, `raid_artifact_probe`, artifact cache, verify-dev-stand e2e. **FM-012 ✅:** OAuth (2026-05-27). **P4 ✅ (2026-05-18):** `poolai_health_load` → [`BENCHMARKS.md`](../performance/BENCHMARKS.md). **FM-019 partial ✅:** pa11y 18 auth + login; `PA11Y_WCAG22=1` — 0 errors (WCAG 2.2 axe); `a11y.yml` на PR для `src/ui/**`. **Далі (§5.1):** FM-003 §4 (BLOCKED); опційно `PA11Y_WCAG22` у CI job. **Не в автопрогоні:** FM-004/006/009/010. Звірка «не зроблено» — [`FUNCTION_MANAGEMENT.md`](../catalog/FUNCTION_MANAGEMENT.md) **§5.3**.
