...
2. **HTTP (FM-005, Partial)** — закінчити **`network/enterprise_api/`** на **`HttpAppError`/`RestError`** замість **`enterprise_err`** / **`api_json_error`** де доречно; не змінювати стабільні **`error.code`** без рішення; **`login`/`refresh`** і **`check_permission`** — окремо.
