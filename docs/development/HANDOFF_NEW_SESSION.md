...
2. **P2** — REST `/raid/*` (workers, events, snapshot, …) уже через **`RaidService`**; **`/api/v1/discovery/*`** — **`DiscoveryService`**; **`/api/v1/topology/*`** — **`TopologyService`**; **`/api/v1/workers`** — **`WorkerPoolService`**; **`/api/v1/rewards/*`** — **`RewardsService`** (поки делегує в модуль `rewards`; майбутнє — `AppState`); опційно далі — тонкі distributed handlers у `raid_distributed_handlers` vs сервіс.
