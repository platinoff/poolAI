...
**Статус проєкту**: стабільний бекенд на Rust (див. [`STABLE_STATE_SUMMARY.md`](../status/STABLE_STATE_SUMMARY.md), rust_ratio **94.91%**)  
**FM §5.1 alignment (PH-S963):** операційна черга спринтів — [`FUNCTION_MANAGEMENT.md`](../catalog/FUNCTION_MANAGEMENT.md) §5.12/§5.14; цей файл — архітектурні P1–P8. Чекбокси нижче **не** дублюють FM-* порядок; єдине джерело пріоритетів розробки — FM **§5.1**.  
...
Таблиця **P1–P7** вище — архітектурні пріоритети й залежності; **конкретна черга робіт** для сесії/спринту — у **`FUNCTION_MANAGEMENT.md` §5.1** та **§5.11** (10 спринтів PH-S37…S46). Коротко той самий порядок (історичний FM-035…036; актуальна PH-черга — §5.11):
...
- [x] **Raft multi-node harness (PH-S06):** `tests/raft_multi_node_harness.rs` + `raft_rpc.rs` — 2-node single-host на ephemeral ports; **`cargo test-raft-ci`** (разом з `raft_wire_integration`).
