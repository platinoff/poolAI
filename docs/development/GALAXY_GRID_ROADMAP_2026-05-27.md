...
**Оновлено:** 2026-06-17 · **Канон черги:** [`FUNCTION_MANAGEMENT.md`](../catalog/FUNCTION_MANAGEMENT.md) §5.12 (**7** відкритих PH-S246…S252)
...
| **Admin i18n slim** | PH-S207…S245 ✅ · band S246, S248, S252 — **відкрито** |
| **Docs sync** | PH-S251 — roadmap + README + INDEX zriz |
| **Rust ratio** | **92.78%** hold **95%** · spirit **96%** |

---

## 3. Черга §5.12 (PH-S246…S252)

| # | Sprint | Тема | Acceptance |
|---|--------|------|------------|
| 1 | **PH-S246** | Admin err hint keys slim | `err.hint*` + insufficientAdmin + accessRequired |
| 2 | **PH-S247** | Pricing provider metrics smoke | catalog lookups/hits + provider errors on `/metrics` |
| 3 | **PH-S248** | VM modal i18n slim | `vm.*` modal keys out of `i18n_core.js` |
| 4 | **PH-S249** | Settlement metrics smoke | settlement pending + cleared on `/metrics` |
| 5 | **PH-S250** | Shard locality metrics smoke | `galaxy_shard_local_hit_ratio` on `/metrics` |
| 6 | **PH-S251** | Docs roadmap sync | GALAXY_GRID_ROADMAP + README + INDEX sprint zriz |
| 7 | **PH-S252** | UI confirm modal slim | `ui.confirm*` + modal glue keys patch |
