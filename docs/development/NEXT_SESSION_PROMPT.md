...
**Порядок:** **S0 диск** (`df -h /s` + `check_target_disk.sh` → `cargo clean` за потреби) → project scan (**спочатку** `rust_diagnostics` / clippy warnings → виправлення в топ смуги) → drain band 91 (Monitoring depth … / close; **без** mid-push) → Speeds (`bash bin/record-test-ci-speed.sh`) · Rust panel (`bash bin/record-rust-diagnostics.sh`) → vision-sync → **один** commit → **`git push` + самарі (завжди кінець сесії)**. Після push — PH-SVC34 GH re-verify (JWT-format `GITHUB_TOKEN` ok). **Не** комітити `certs/*.pem`, `.env`, `data/audit/*`. **Не** валідувати довжину `ghs_*`.
...
| **PH-S1550** | `monitoring` store/wire slice |
| **PH-S1551** | `monitoring` API contracts |
| **PH-S1552** | `monitoring` admin/ops glue |
| **PH-S1553** | Stand smoke export |
| **PH-S1554** | `poolai-loc-audit` monitoring zriz |
| **PH-S1555** | Docs canon sync |
| **PH-S1556** | vision-sync --check |
| **PH-S1557** | Ratio hold advisory |
| **PH-S1558** | Band close → `galaxy_horizon_s1549_integration` |

Канон: next project-scan band from [`PH_S_MASTER_BACKLOG_1000.md`](./PH_S_MASTER_BACKLOG_1000.md) (band 91 `PH-S1549…S1558`)
