...
**Порядок:** drain band 89 (Policies ratio-advisory aggregate). Після drain — **`bash bin/record-test-ci-speed.sh`** → Speeds · **`bash bin/record-rust-diagnostics.sh`** → Rust panel ([`RUST_DIAGNOSTICS.md`](./RUST_DIAGNOSTICS.md); CI job `rust-diagnostics`). Потім vision-sync / push. Після push — PH-SVC34 GH re-verify (JWT-format `GITHUB_TOKEN` ok). **Не** комітити `certs/*.pem`, `.env`, `data/audit/*`. **Не** валідувати довжину `ghs_*`.
