...
param(
    ...
    [switch]$TenantLocAudit,
    [switch]$TenantDocsCanon,
    [switch]$TenantVisionSync,
    [switch]$TenantRatioAdvisory,
    [int]$Port = 8080,
...
function Invoke-Quick {
    ...
    if ($TenantStore) {
        Write-Host "Running poolai-loc-audit --tenant-store (PH-S1162)..."
        & $MsysWrapper cargo run --quiet --bin poolai-loc-audit -- --tenant-store
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
    if ($TenantApi) {
    ...
    if ($TenantLocAudit) {
        Write-Host "Running poolai-loc-audit --tenant-loc-audit (PH-S1202)..."
        & $MsysWrapper cargo run --quiet --bin poolai-loc-audit -- --tenant-loc-audit
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
    if ($TenantDocsCanon) {
        Write-Host "Running poolai-loc-audit --tenant-docs-canon (PH-S1212)..."
        & $MsysWrapper cargo run --quiet --bin poolai-loc-audit -- --tenant-docs-canon
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
    if ($TenantVisionSync) {
        Write-Host "Running poolai-loc-audit --tenant-vision-sync (PH-S1222)..."
        & $MsysWrapper cargo run --quiet --bin poolai-loc-audit -- --tenant-vision-sync
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
    if ($TenantRatioAdvisory) {
        Write-Host "Running poolai-loc-audit --tenant-ratio-advisory (PH-S1232)..."
        & $MsysWrapper cargo run --quiet --bin poolai-loc-audit -- --tenant-ratio-advisory
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
}
