...
Виконуй **по одній команді** або копіюй блок цілом. **Агент/Composer**: завжди формуй subject + кілька рядків Summary в тілі коміта (див. п.3a).
...
git add -f .cursor/hooks.json .cursor/CHANGELOG.md .cursor/README.md .cursor/rules/ .cursor/commands/ .cursor/skills/
git status -sb
...
### 3a. Commit з розгорнутим Summary (обов’язково для автоматичної роботи)

Кілька `-m` у Git — окремі абзаци в історії. Мінімум:

1. `type(scope): subject`
2. `Summary:` + буллети: **що змінено**, **які команди прогнані**, **особливості** (диск, feature flags).
