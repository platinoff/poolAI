...
### `target/` і ліміт дискового простору

Щоб **не переповнювався диск** і не падали лінкер / тести через `ENOSPC` або фрагментацію вільного місця:

- **Політика (м’які пороги за замовчуванням):** тримати **≥ 12 GiB** вільних на томі, де лежить репозиторій (або `CARGO_TARGET_DIR`); каталог **`target/`** доречно тримати **≤ ~48 GiB** для цього монорепо — інакше `cargo clean` або окремий том під `CARGO_TARGET_DIR`.
- **Перевірка перед важкими прогонами:** `bash scripts/check_target_disk.sh` (лише попередження) або з жорстким фейлом: `bash scripts/check_target_disk.sh --enforce` / `POOLAI_ENFORCE_DISK_LIMIT=1`. Пороги: `POOLAI_MIN_FREE_DISK_GB`, `POOLAI_MAX_TARGET_DIR_GB` (див. шапку скрипта).
- **Зменшення тиску на диск:** `CARGO_INCREMENTAL=0` для разових повних матриць; `-j 1` для лінкера на Windows (уже в доках); не тримати одночасно кілька величезних `target/` на одному тому.
