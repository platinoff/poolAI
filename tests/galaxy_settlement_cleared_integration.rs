//! PH-S187: Galaxy settlement cleared total — grid result Cleared path → Prometheus scrape.
...
use poolai::grid::galaxy_settlement_metrics::{
    reset_settlement_metrics_for_test, METRIC_SETTLEMENT_CLEARED_TOTAL,
};
...
static SETTLEMENT_CLEARED_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

fn settlement_cleared_lock() -> std::sync::MutexGuard<'static, ()> {
    SETTLEMENT_CLEARED_LOCK
        .get_or_init(|| Mutex::new(()))
        .lock()
        .expect("galaxy settlement cleared integration lock")
}
...
#[tokio::test]
async fn grid_result_trust_eligible_increments_settlement_cleared_on_scrape() {
    let _lock = settlement_cleared_lock();
    ...
    assert!(metrics.contains(METRIC_SETTLEMENT_CLEARED_TOTAL));
    assert!(metrics.contains(&format!("{METRIC_SETTLEMENT_CLEARED_TOTAL} 1")));
