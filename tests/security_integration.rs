...
use poolai::network::auth::{generate_token, validate_token, UserRole, authenticate_user, AuthRequest};
...
async fn test_token_generation_fallback() {
    ...
    #[cfg(not(feature = "jwt"))]
    {
        assert!(token.starts_with("dev_token_"), "Fallback token should start with 'dev_token_'");
    }
...
async fn test_token_expiration() {
    ...
    assert!(result.is_ok(), "Token should be valid immediately after generation");
...
async fn test_authentication_flow() {
    ...
    assert!(response.is_ok(), "Authentication should succeed for valid credentials");
...
async fn test_authentication_invalid_credentials() {
    ...
    let response = authenticate_user(auth_req).await;
    assert!(response.is_err(), "Authentication should fail for invalid credentials");
}
