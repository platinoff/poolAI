//! PH-S180: Galaxy verification match total — grid result path → Prometheus scrape.
...
use poolai::grid::galaxy_verification_metrics::{
    reset_verification_match_metrics_for_test, METRIC_VERIFICATION_MATCH_TOTAL,
};
...
static VERIFICATION_MATCH_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

fn verification_match_lock() -> std::sync::MutexGuard<'static, ()> {
    VERIFICATION_MATCH_LOCK
        .get_or_init(|| Mutex::new(()))
        .lock()
        .expect("galaxy verification match integration lock")
}
...
#[tokio::test]
async fn grid_result_match_increments_verification_metric_on_scrape() {
    let _lock = verification_match_lock();
    ...
    let (result_status, _) = request_json(
        ...
        Some(json!({
            ...
            "lease_epoch": epoch,
            "metrics": { "verification_verdict": "match" },
            "source_peer_id": "tg-edge"
        })),
    )
    .await;
    ...
    assert!(metrics.contains(METRIC_VERIFICATION_MATCH_TOTAL));
    assert!(metrics.contains(&format!("{METRIC_VERIFICATION_MATCH_TOTAL} 1")));
