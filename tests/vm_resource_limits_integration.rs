...
use poolai::vm::{VmManager, VmResources, VmIsolation, ResourceLimits, ResourceLimiter, PlatformResourceLimiter};
...
async fn test_resource_limits_validation() {
    ...
    let result = manager.apply_resource_limits(&mut command, instance.id).await;
...
async fn test_resource_limits_support_detection() {
    ...
    assert!(supported, "Resource limits should be supported on Windows/Linux");
    
    #[cfg(not(any(target_os = "windows", target_os = "linux")))]
    assert!(!supported, "Resource limits should not be supported on other platforms");
}
...
async fn test_get_instance_resource_usage_not_implemented() {
    ...
    assert!(result.is_err(), "Resource usage query should fail until process spawning is implemented");
}
