...
async fn test_sqlite_persistence_record_metric() -> Result<(), AppError> {
    ...
    let history = manager
        .get_metric_history(Some("cpu_usage"), None, None, Some(tenant_id), None)
        .await?;
...
async fn test_sqlite_persistence_multiple_metrics() -> Result<(), AppError> {
    ...
    // Query all metrics
    let history = manager
        .get_metric_history(None, None, None, Some(tenant_id), None)
        .await?;

    assert_eq!(history.len(), 10);

    // Query specific metric
    let metric_0_history = manager
        .get_metric_history(Some("metric_0"), None, None, Some(tenant_id), None)
        .await?;
...
async fn test_sqlite_persistence_limit_filter() -> Result<(), AppError> {
    ...
    // Query with limit
    let history = manager
        .get_metric_history(Some("test_metric"), None, None, Some(tenant_id), Some(5))
        .await?;
...
async fn test_sqlite_persistence_tenant_filter() -> Result<(), AppError> {
    ...
    let tenant_1_history = manager
        .get_metric_history(Some("test_metric"), None, None, Some(tenant_1), None)
        .await?;

    assert_eq!(tenant_1_history.len(), 5);

    // Query tenant_2 metrics
    let tenant_2_history = manager
        .get_metric_history(Some("test_metric"), None, None, Some(tenant_2), None)
        .await?;
...
async fn test_sqlite_persistence_tags_serialization() -> Result<(), AppError> {
    ...
    let history = manager
        .get_metric_history(Some("test_metric"), None, None, None, None)
        .await?;
    ...
    assert_eq!(
        history[0].tags.get("region"),
        Some(&"us-west-2".to_string())
    );
    assert_eq!(
        history[0].tags.get("environment"),
...
async fn test_sqlite_persistence_cleanup_old_metrics() -> Result<(), AppError> {
    ...
    let old_history = manager
        .get_metric_history(Some("old_metric"), None, None, Some(tenant_id), None)
        .await?;
    ...
    let recent_history = manager
        .get_metric_history(Some("recent_metric"), None, None, Some(tenant_id), None)
        .await?;
