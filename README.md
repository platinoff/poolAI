...
- **P3**: `http_status_for_app_error` + Axum **`IntoResponse`** для **`AppError`** / **`HttpAppError`** (`ErrorContext` / override статусу); див. `src/network/json_errors.rs`, реекспорт **`HttpAppError`** у `network/api/common.rs`.
