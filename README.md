...
## Galaxy docs vision (інтерактивна карта доків — проєкт GSV)

Після серії спринтів **Galaxy Grid** (концепт у [`docs/concept/POOLAI_GALAXY_GRID.md`](docs/concept/POOLAI_GALAXY_GRID.md)) у проєкті GSV з’явилася **жива карта**: як пов’язані концепт, HANDOFF, FM і код (`galaxy_fee_split`, virtual nodes тощо).

> **Важливо:** цей розділ належить проєкту **GSV**, а не головному poolAI репози터ію. Головні docs poolAI залишаються у `docs/`.
...
**Таксономія каталогу `docs/`** — [`docs/STRUCTURE.md`](docs/STRUCTURE.md). **Правила агента для доків** — [`.cursor/rules/documentation.mdc`](.cursor/rules/documentation.mdc).
...
Зрілий **MVP і модулі Stage 1–3** здані й покриті тестами; **Galaxy Grid wire:** PH-S65…S1908 ✅ (phase F Ratio96 stand smoke). **Rust ratio:** **95.15%** (hold **95%** advisory). **Vision:** manifest rev **495**. **§5.12:** **0** відкритих (band 114 ✅ · band 115 next) · tech **PH-SVC75…SVC84** Cursor **3.13.21** ✅ · наступна сесія **`абракадабра`** (band 115) — [`NEXT_SESSION_PROMPT.md`](docs/development/NEXT_SESSION_PROMPT.md).
...
### Next Focus (2026-08-05)

**Product:** наступна сесія **`абракадабра`** → drain band 113 **PH-S1769…S1778** (project scan наступний).

**GSV:** окремий git-репозиторій **`S:\rust\GSV`** — vision server з боксами (95–100% Rust / 0–5% wasm) · вказівник [`GSV/README.md`](GSV/README.md) · PoolAI vision-канон [`docs/vision/`](docs/vision/README.md).
...
**§5.12:** **0** відкритих (band 114 ✅ · band 115 next) · vision **rev 495** · last **PH-S1908** · next **PH-S1899**.
