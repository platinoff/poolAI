---
name: abracadabra
description: Trigger word «абракадабра» — start of a VDT-drain session. FIRST ask the owner to choose the project target (poolai | gsv) via the question tool, THEN run the full project-scan → drain cycle for the chosen project according to its roadmaps/concepts. Use only when the owner literally writes «абракадабра» in a new session.
---
...
When the owner writes `абракадабра`, **before anything else** ask which project to drain:

| Option | Project | Canon docs | Flow |
|--------|---------|-----------|------|
| **poolai** | PoolAI (this repo root, `src/`, `tests/`) | `docs/development/NEXT_SESSION_PROMPT.md`, `docs/catalog/FUNCTION_MANAGEMENT.md` §5.12 | S0 disk → project scan → drain band (FM §5.12) → vision close → test → one commit + push |
| **gsv** | GSV subproject (`GSV/` — separate Rust-first project, own `Cargo.toml`) | `GSV/docs/NEXT_SESSION_PROMPT.md`, `GSV/docs/GSV_ROLES.md`, `GSV/docs/gsv/GSV_TECH_ROADMAP.md` | S0 disk (GSV target) → project scan (warnings first) → drain next band (FM §5.102 / GSV roadmap) → Speeds + Rust panel → vision-sync → one commit + push |

Use the `question` tool with the two options so the owner picks in one click.
...
- `AGENTS.md` § «Тригер абракадабра» (canon, repo root)
- `.cursor/rules/poolai-session-iteration.mdc` § «Тригер абракадабра»
